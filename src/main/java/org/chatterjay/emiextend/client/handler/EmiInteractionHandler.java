...
import org.chatterjay.emiextend.client.ClientOnlyNetworking;
...
public final class EmiInteractionHandler {
                        ...
                        if (matchAll) {
                            ...
                            for (int i = 0; i < mc.player.getInventory().items.size(); i++) {
                                ...
                                if (!s.isEmpty() && ItemStack.isSameItemSameComponents(s, carried)) {
                                    ClientOnlyNetworking.sendEmiLinkPacket(new AEDepositPacket(s.copy(), i));
                                }
                            }
                        } else {
                            ClientOnlyNetworking.sendEmiLinkPacket(new AEDepositPacket(carried.copy(), -1));
    ...
    private static boolean tryExtractHoveredAeStack(ItemStack stack) {
        ...
        ClientOnlyNetworking.sendEmiLinkPacket(new AEExtractPacket(stack.copyWithCount(1), 1));
        org.chatterjay.emiextend.util.ModLogger.debug(
    ...
    private static boolean tryRequestHoveredAeStack(ItemStack stack) {
        ...
        ClientOnlyNetworking.sendEmiLinkPacket(new AEAutocraftRequestPacket(stack.copyWithCount(1), 1));
        org.chatterjay.emiextend.util.ModLogger.debug(
