...
- 优化 AE 网络角标查询：打开 AE 终端时默认只查询库存角标，不再默认查询昂贵的可合成角标，并新增初始扫描数量上限配置以降低大型 AE 网络卡顿。
- 服务端未安装 EmiLink 时提前停用 AE 网络缓存轮询和角标访问检查，减少纯客户端安装场景下的 AE/EMI 渲染开销。
- 默认关闭 EMI 索引侧栏的生物物品分组重排，并为该重排结果增加缓存，避免大型整合包中侧栏显示时反复全量扫描 EMI 索引列表。
- AE 快捷合成 preflight 改用一次性临时查询缓存：只查询当前 BoM 树需要的输入和输出，任务完成、取消或转交 AE 自动合成后立即释放结果，避免污染长期 EMI 角标/tooltip 缓存。

### 修复

- 修复 EMI 收藏夹空白格会 hover 高亮的问题，同时允许空白格点击继续交给底层 GUI。
- 修复创造模式下光标物品在 EMI 收藏夹真实物品格上无法使用一键丢弃的问题，并避免收藏夹空白格误触发删除。
- 过滤 EMI 搜索源中的空 ingredient，避免搜索线程刷 `NullPointerException`。
- 修复 SFM 代码编辑器渲染后遗留的 GUI 渲染状态导致 EMI 图标显示为空气、但悬停信息仍存在的问题。

## [1.1.16-test.1] - 2026-07-20
