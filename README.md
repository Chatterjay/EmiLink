...
- `quick_craft`：BOM 自动工作台合成开关和 AE 每 tick 合成批次数。
...
BOM 递归自动工作台合成已恢复。BOM 界面按钮和收藏夹中的合成入口都会按叶子节点到最终产物的顺序执行；AE、BD 和普通工作台路径都会按批次处理，以缩短大量合成的等待时间。`quick_craft.batchesPerTick` 可限制每 tick 的批次数。
